...
internal class InteractionHandlerService : DiscordClientService
{
    ...
    public InteractionHandlerService(DiscordSocketClient client, ILogger<DiscordClientService> logger, InteractionService handler, IServiceProvider provider) : base(client, logger)
    {
        _handler = handler;
        _provider = provider;
    }
